...
#### 1. Unifi App에서 createPayment API 호출

사용자가 구매를 요청하면 <mark style="background-color:purple;">유니파이 앱 클라이언</mark>트가 상품 정보를 <mark style="background-color:purple;">유니파이 앱</mark> 서버로 전송합니다.\
<mark style="background-color:purple;">유니파이 앱 서버</mark>는 <mark style="background-color:green;">유니파이 결제</mark> 서버의 `createPayment` API를 호출합니다.\
`createPayment` API는 \`{ id:\<payment\_id>

}\`\</payment\_id> 형식의 결제 ID로 응답합니다\<payment\_id>
...
<mark style="background-color:purple;">Unifi 앱 클라이언트는</mark>

getPaymentProvider 메서드를 사용하여 SDK에서 `PaymentProvider` 인스턴스를 가져옵니다.
...
웹훅을 사용할 경우, 결제 상태가 변경될 때마다 <mark style="background-color:green;">Unifi 결제 서버</mark>

는 `createPayment` API 매개변수에 지정된 `paymentStatusChangeCallbackUrl`로 업데이트를 전송합니다.\
업데이트 요청은 HTTP <mark style="background-color:green;">POST</mark>

로 전송되며, 본문에 다음과 같은 형식으로 업데이트된 결제 상태가 포함됩니다:
...
웹훅 이벤트에 대한 응답으로 HTTP `200 OK` 상태가 반환되지 않으면, 이벤트는 지수적으로 증가하는 간격(**1초, 2초, 4초, 8초**)으로 최대 4회까지 재시도됩니다.

모든 재시도 실패 후, 결제 생성 시 `lockUrl`이 지정된 경우 <mark style="background-color:green;">Unifi 결제 서버</mark>

는 HTTP <mark style="background-color:green;">POST</mark>

로 잠금 해제 요청을 전송합니다. 본문에는 paymentId와 itemIdentifiers가 포함됩니다.
...
<mark style="background-color:green;">**POST**</mark><mark style="background-color:green;">\*\*</mark>
...
X-Client-Secret <mark style="color:red;">\*필수</mark>\
문자열\
&#xNAN;_(헤더)_<br>
...
<table data-full-width="false"><thead><tr><th>예시 값</th><th>스키마</th></tr></thead><tbody><tr><td><pre class="language-javascript"><code class="lang-javascript">{
...
X-Client-Id <mark style="color:red;">\*필수</mark>\
문자열\
&#xNAN;_(헤더)_
...
id <mark style="color:red;">\*필수</mark>
...
</code></pre></td></tr><tr><td>404</td><td><p>예시 값</p><pre class="language-javascript"><code class="lang-javascript">{
    "code": 1002,
    "detail": "Not found payment",
    "cause": null
}
...
<table><thead><tr><th>예제 값</th><th>스키마</th></tr></thead><tbody><tr><td><pre class="language-javascript"><code class="lang-javascript">{
    id: "{payment_id}" 
}
</code></pre></td><td><pre class="language-javascript"><code class="lang-javascript">{
    id*: String
...
<table><thead><tr><th width="159.82525634765625">HTTP 상태 코드</th><th>설명</th></tr></thead><tbody><tr><td>200</td><td>해당 없음</td></tr><tr><td>403</td><td><p>예제 값</p><pre class="language-javascript"><code class="lang-javascript">{
    "code": 1004,
...
`paymentProvider.startPayment()` 호출 시 사용자에게 거래 창을 표시하고 결제 프로세스를 시작합니다.
...
`lockUrl`은 구매 중인 항목을 일시적으로 잠그거나 예약하기 위해 호출되는 서버 엔드포인트입니다. 이는 일반적으로 **한정 수량** 또는 **NFT** 항목에 대해 초과 판매를 방지하고 현재 결제 진행 중일 때 다른 사용자가 해당 항목을 구매하지 못하도록 보장하기 위해 필요합니다.
...
`lockUrl`로 요청이 이루어졌으나 200 응답을 받지 못한 경우, 결제는 즉시 취소되고 자동으로 실패로 표시됩니다.
...
`unlockUrl`은 **결제가 실패하거나 취소되거나 시간 초과될 때** 호출되는 서버 엔드포인트로, 이전에 잠겨 있던 아이템의 잠금을 해제하여 다른 사용자가 다시 구매할 수 있도록 합니다.

**결제 생성 요청**에 `unlockUrl` 매개변수가 포함된 경우, 결제 실패 관련 이벤트가 발생하면 이 엔드포인트로 `POST` 요청이 전송됩니다.

결제가 성공적으로 완료된 경우(`status === CONFIRMED`), `unlockUrl`은 **호출되지 않습니다**. 대신, 일반적으로 별도의 프로세스( (`finalize` API 또는 내부 로직).

`unlockUrl`로 요청이 이루어졌으나 200 응답을 받지 못한 경우, 1초, 2초, 4초, 8초 간격으로 최대 5회 재시도됩니다. 아이템이 잠긴 상태로 유지되는 것을 방지하려면, 잠금 해제를 다시 시도하는 백그라운드 작업을 구현하는 것이 권장됩니다.
...
결제 상태가 **CREATED**인 경우를 제외하고, 결제 상태가 변경될 때마다 이벤트가 발생합니다.
