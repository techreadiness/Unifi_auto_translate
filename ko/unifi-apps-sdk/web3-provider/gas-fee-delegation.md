...
#### 카이아 수수료 위임은 세 가지 주요 구성 요소로 이루어집니다:

1. 발신자: 거래를 제출하는 계정 (from)
2. Unifi Apps: 사용자가 서명한 거래를 수수료 지불자 서버로 중계하는 중계자
3. 수수료 지불자 서버: 수수료 지불자로 서명하고 서명된 거래를 네트워크로 전송하며 영수증을 호출자에게 반환

#### Kaia 수수료 위임의 운영 절차는 다음과 같습니다:

1. 발신자가 거래를 생성합니다.
2. 송신자는 트랜잭션에 수수료 지불자의 주소를 지정합니다.
3. 송신자는 트랜잭션에 서명합니다.
4. 송신자는 서명된 트랜잭션을 유니피 앱스 백엔드로 전송합니다.
5. 백엔드는 수수료 지불 서버에 트랜잭션 서명을 요청합니다.
6. 수수료 지불 서버는 수수료 지불자로서 트랜잭션에 서명하고 네트워크로 전송한 후, 트랜잭션 영수증을 받아 송신자에게 반환합니다.
...
1. SDK
   1. ethers-ext는 kaia-sdk의 일부로 설치되어야 합니다.
   2. ethers-ext [시작하기](https://docs.kaia.io/ko/references/sdk/ethers-ext/getting-started/)
2. 지갑
   1. 수수료 위임은 다음 지갑에서만 지원됩니다.
      1. [kaia-wallet](https://www.kaiawallet.io/) 모바일/확장 프로그램, [Unifi-wallet](https://wallet.dappportal.io/) liff/웹
   2. Unifi Apps 프론트엔드는 지갑을 통해 사용자 서명 트랜잭션을 가져와야 합니다.
